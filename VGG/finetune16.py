...
batch_size = 16
num_classes = 2  # 类别标签
train_layer = ['fc8', 'fc7', 'fc6', 'conv5_3', 'conv5_2', 'conv5_1', 'conv4_3', 'conv4_2', 'conv4_1',
               'conv3_3', 'conv3_2', 'conv3_1', 'conv2_2', 'conv2_1', 'conv1_2', 'conv1_1']

filewriter_path = "D:/machine_learning/vgg/tensorboard"  # 存储tensorBoard文件
checkpoint_path = "D:/machine_learning/vgg/checkpoints"  # 训练好的模型和参数存放目录
...
x = tf.placeholder(tf.float32, [batch_size, 224, 224, 3], name="x")
y = tf.placeholder(tf.float32, [batch_size, num_classes], name="y")
learning_rate = tf.placeholder(tf.float32)
keep_prob = tf.placeholder(tf.float32)

# 图片数据通过VGG16网络处理
model = Vgg16(bgr_image=x, num_class=num_classes, vgg16_npy_path='D:/machine_learning/vgg/vgg16.npy')
score = model.fc8
y_ = model.prob
...
#    with tf.device("/gpu:0"):
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_iter):
            ...
            if epoch < 3:
                _, train_loss = sess.run([train_op, loss], feed_dict={x: img_batch, y: label_batch, learning_rate: 1e-3})
            else:
                _, train_loss = sess.run([train_op, loss], feed_dict={x: img_batch, y: label_batch, learning_rate: 1e-4})
