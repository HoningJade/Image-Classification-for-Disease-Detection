...
def GoogLeNet(im_height=224, im_width=224, class_num=1000, aux_logits=False):
    ...
    if aux_logits:
        model = models.Model(inputs=input_image, outputs=[aux1, aux2, aux3])  # 判断是否符合辅助分类器
    else:
...
class Inception(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        outputs = layers.concatenate([branch1, branch2, branch3, branch4])   # 拼接深度
        return outputs
...
class InceptionAux(layers.Layer):
    def __init__(self, num_classes, **kwargs):
        ...
        self.fc1 = layers.Dense(1024, activation="relu")   # 全连接层
    ...
    def call(self, inputs, **kwargs):
        ...
        # N x 128 x 4 x 4
        x = layers.Flatten()(x)
        x = layers.Dropout(rate=0.3)(x)  # 随机失活，原论文是0.7, here 0.3
        # N x 2048
        x = self.fc1(x)
